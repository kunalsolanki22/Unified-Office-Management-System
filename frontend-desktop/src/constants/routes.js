export const ROUTES = {
    HOME: '/',
    LOGIN: '/login',
    UNAUTHORIZED: '/unauthorized',

    // Super Admin
    SUPER_ADMIN_DASHBOARD: '/super-admin/dashboard',
    SUPER_ADMIN_ADMINS: '/super-admin/admins',
    SUPER_ADMIN_ATTENDANCE: '/super-admin/attendance',
    SUPER_ADMIN_HOLIDAYS: '/super-admin/holidays',
    SUPER_ADMIN_ACTIONS: '/super-admin/actions',
    SUPER_ADMIN_ANALYTICS: '/super-admin/analytics',

    // Admin
    ADMIN_DASHBOARD: '/admin/dashboard',
    ADMIN_USER_DIRECTORY: '/admin/user-directory',
    ADMIN_ATTENDANCE: '/admin/attendance',
    ADMIN_HOLIDAYS: '/admin/holidays',
    ADMIN_ACTION_HUB: '/admin/action-hub',
    ADMIN_MY_ATTENDANCE: '/admin/my-attendance',

    // Manager
    MANAGER_DASHBOARD: '/manager/dashboard',

    // IT Hardware Manager
    HARDWARE_DASHBOARD: '/manager/hardware/dashboard',
    HARDWARE_REQUESTS: '/manager/hardware/requests',
    HARDWARE_ASSETS: '/manager/hardware/assets',
    HARDWARE_VENDORS: '/manager/hardware/vendors',

    // Parking Manager
    PARKING_DASHBOARD: '/manager/parking/dashboard',
    PARKING_SLOTS: '/manager/parking/slots',
    PARKING_REQUESTS: '/manager/parking/requests',

    // Team Lead
    TEAM_LEAD_DASHBOARD: '/team-lead/dashboard',
};