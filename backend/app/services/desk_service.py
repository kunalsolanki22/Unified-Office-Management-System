...
class DeskService:
    ...
    async def check_room_booking_overlap(
        ...
        Only checks against CONFIRMED bookings, not PENDING ones.
        This allows multiple users to request the same time slot,
        with the manager deciding which one to approve.
        """
    ...
    async def create_room_booking(
        ...
        # NOTE: We intentionally do NOT check for overlapping bookings here.
        # Multiple users can request the same room/time slot.
        # The conference manager will decide which request to approve.
        # Overlap validation is performed only when approving a booking.
