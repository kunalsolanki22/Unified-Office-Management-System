...
class ConversationState:
    ...
    # Support for multiple pending actions in multi-agent mode
    pending_actions: Optional[Dict[str, Dict[str, Any]]] = None  # agent_type -> context
    message_count: int = 0
...
class OrchestratorResponse:
    ...
    tokens_used: Optional[int] = None  # Total LLM tokens used for this request
...
class Orchestrator:
    ...
    def process_message(self, user_message: str) -> OrchestratorResponse:
        ...
        try:
            ...
            # Initialize routing_result as None for pending action flows
            routing_result = None
            
            # Step 1: Check if we have pending actions from multi-agent mode
            if self.state.pending_actions and len(self.state.pending_actions) > 0:
                logger.info(f"Processing response to multiple pending agents: {list(self.state.pending_actions.keys())}")
                response = self._process_pending_actions_response(user_message)
            elif self.state.pending_action and self.state.current_agent:
                # Single pending action (backwards compatibility)
            ...
            # Step 5: Save to database (routing_result may not exist for pending actions)
            if routing_result is None:
                # Create a placeholder routing result for logging
                routing_result = RoutingResult(
                    selected_agent=self.state.current_agent or AgentType.GENERAL,
                    confidence=1.0,
                    detected_intent="Multi-agent followup",
                    reasoning="User response to pending actions",
                    alternative_agents=[],
                    is_greeting=False,
                    is_farewell=False
                )
    ...
    def _handle_general(self, routing_result: RoutingResult, user_message: str) -> OrchestratorResponse:
        ...
        system_prompt = f"""You are a professional, helpful AI Employee Services Assistant.
...
## STRICT RULES
1. Be professional, warm, and helpful - NO sarcasm or jokes
2. If they ask "who am i", "my info" → Give their details politely
3. If they greet you → Greet back warmly and offer to help
4. If they ask for help → Explain what you can do clearly
5. Be concise and friendly
6. DO NOT hallucinate or make up information
7. NEVER be sarcastic, rude, or make jokes about the user"""
    ...
    def _process_with_agent(self, routing_result: RoutingResult, user_message: str) -> OrchestratorResponse:
        ...
        if result.needs_followup:
            ...
            self.state.pending_actions = None  # Clear multi-agent pending too
        ...
        return OrchestratorResponse(
            ...
            tokens_used=result.tokens_used,  # Track actual LLM tokens used
    ...
    def _process_with_multiple_agents(self, routing_result: RoutingResult, user_message: str) -> OrchestratorResponse:
        ...
        """
        # Clear any previous pending actions when starting fresh multi-agent processing
        ...
        all_api_details = []
        total_tokens_used = 0  # Track tokens across all agents
        ...
        for agent_info in routing_result.selected_agents:
            ...
            if result.needs_followup and result.followup_context:
                ...
                # Store ALL pending contexts for multi-agent mode
                if self.state.pending_actions is None:
                    self.state.pending_actions = {}
                self.state.pending_actions[agent_type.value] = {
                    "context": result.followup_context,
                    "intent": agent_intent
                }
                # Also set the first agent as current for backwards compatibility
        ...
        return OrchestratorResponse(
            ...
            tokens_used=total_tokens_used if total_tokens_used > 0 else None,  # Total tokens across all agents
            # Execution logging details
            user_input=user_message,
    ...
    def _process_pending_actions_response(self, user_message: str) -> OrchestratorResponse:
        """
        Process user response when there are multiple pending actions from multi-agent mode.
        Each pending agent processes the user's response with their stored context.
        """
        agent_results = []
        all_prompts = []
        all_llm_responses = []
        ...
        pending_agents = list(self.state.pending_actions.items())
        logger.info(f"Processing user response for {len(pending_agents)} pending agents")
        
        for agent_type_str, pending_info in pending_agents:
            agent_type = AgentType(agent_type_str)
            agent = self._get_agent(agent_type)
            if not agent:
                logger.warning(f"Could not get agent for type: {agent_type_str}")
                continue
            
            logger.info(f"Processing pending action for {agent_type_str} agent")
            
            # Get the original intent for this agent
            original_intent = pending_info.get("intent", "")
            
            # Build a focused user message that includes original context
            # This helps the agent understand what the user is responding to
            if original_intent:
                focused_message = f"[Original request: {original_intent}] User response: {user_message}"
            else:
                focused_message = user_message
            
            # Build agent context with the stored pending action
            context = AgentContext(
                user_message=focused_message,
                conversation_history=self.state.conversation_history[-self.settings.max_conversation_history:],
                user_info=self.state.user_info,
                pending_action=pending_info.get("context"),
                session_data={"session_id": str(self.state.session_id)}
            )
            ...
            agent_results.append({
                "agent": agent_type.value,
                "intent": pending_info.get("intent", ""),
                "message": result.message,
                "success": result.success,
                "api_response": result.api_response.to_dict() if result.api_response else None,
                "needs_followup": result.needs_followup,
            ...
            # If agent still needs followup, keep it pending
            if result.needs_followup and result.followup_context:
                needs_any_followup = True
                new_pending_actions[agent_type.value] = {
                    "context": result.followup_context,
                    "intent": pending_info.get("intent", "")
                }
                logger.info(f"Agent {agent_type.value} still needs followup")
        
        # Update pending actions
        if new_pending_actions:
            self.state.pending_actions = new_pending_actions
            # Set current_agent to first pending for backwards compatibility
            first_agent = list(new_pending_actions.keys())[0]
            self.state.current_agent = AgentType(first_agent)
            self.state.pending_action = new_pending_actions[first_agent]["context"]
        else:
        ...
        return OrchestratorResponse(
            message=combined_message,
            agent_used=",".join([a["agent"] for a in agent_results]),
            action_type="multi_agent_followup",
            success=overall_success,
            needs_followup=needs_any_followup,
            metadata={
                "agent_results": agent_results,
...
"""
        ...
        system_prompt = f"""You are a professional, helpful AI Employee Services Assistant combining multiple action results.
The user asked for multiple things in one message, and different agents handled each request.

## STRICT TONE RULES
- Be professional, warm, and helpful
- NO sarcasm, NO roasting, NO jokes about user's requests
- Be concise and factual
- Show empathy for any failures
{options_instructions}
## User Info
...
## Guidelines
1. Summarize what was accomplished (or failed) for each action clearly
2. If any result has a NUMBERED LIST of options, include the COMPLETE list
3. Be concise but informative
4. If something failed, explain politely what's needed
5. End with prompting them to make a selection if options are shown
6. NEVER be sarcastic or make jokes about the user

Respond with ONLY your combined message - no JSON, no formatting markers."""
            ...
            with db_manager.session_scope() as db_session:
                ...
                if response.prompt_sent or response.llm_response:
                    ...
                    execution_log_repo.create(
                        ...
                        llm_tokens_used=response.tokens_used,  # Token count from LLM
                        error_message=None
                    )
